...
/**
 * Sohbette gerçek zamanlı ses tanımayı etkinleştirerek komutları sesinizi kullanarak yürütmenize olanak tanır.
 * Not: Şu anda yalnızca İngilizce dilini desteklemektedir.
 * YAPILACAKLAR bunu çoklu dil haline getiriyor...
 */
const commands = {
    shareRoom: 'room',
    hideMe: 'hide me',
...
if (speechRecognition) {
    ...
    console.log('Konuşma tanıma', recognition);

    recognition.onstart = function () {
        console.log('Konuşma tanıma başlatıldı');
        hide(chatSpeechStartButton);
        show(chatSpeechStopButton);
        setColor(chatSpeechStopButton, 'lime');
        userLog('info', 'Konuşma tanıma başlatıldı', 'top-end');
    };
    ...
    recognition.onerror = function (event) {
        ...
        userLog('error', `Konuşma tanıma hatası ${event.error}`, 'top-end', 6000);
    };

    recognition.onend = function () {
        console.log('Konuşma tanıma durduruldu');
        show(chatSpeechStartButton);
        hide(chatSpeechStopButton);
        setColor(chatSpeechStopButton, 'white');
        userLog('info', 'Konuşma tanıma durduruldu', 'top-end');
    };

    isWebkitSpeechRecognitionSupported = true;
    console.info('Tarayıcı webkitSpeechRecognition ı destekler');
} else {
    console.warn('Bu tarayıcı webkitSpeechRecognition ı desteklemiyor');
}
...
function printCommand(command) {
    console.log('Tespit Edildi', { command: command });
}
