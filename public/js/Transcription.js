...
class Transcription {
        ...
        if (this.isSupported()) {
            ...
            this.transcription.onstart = function () {
                ...
                !transcription.isPersistentMode
                    ? userLog('info', 'Çeviri başladı', 'top-end')
            ...
            this.transcription.onaudiostart = () => {
                console.log('Çeviri sesinizi yakalamaya başladı');
            };

            this.transcription.onaudioend = () => {
                console.log('Çeviri sesinizi yakalamayı durdurdu');
            };

            this.transcription.onerror = function (event) {
                console.error('Çeviri hatası', event.error);
                if (!transcription.isPersistent || !transcription.isPersistentMode)
                    userLog('error', `Çeviri hatası ${event.error}`, 'top-end', 6000);
            };

            this.transcription.onend = function () {
                console.log('Çeviri durdu');
                ...
                if (
                    ...
                    userLog('info', 'Çeviri durdu', 'top-end');
                }
            };

            this.isTranscriptionSupported = true;
            console.info('Bu Tarayıcı Çeviriyi destekliyor');
        } else {
            console.warn(
                'Bu tarayıcı Çeviriyi desteklemiyor, desteklenen tarayıcılara göz atın: https://developer.mozilla.org/en-US/docs/Web/API/Web_Speech_API#browser_compatibility',
            );
        }
    }
    ...
    sendTranscript(transcriptionData) {
        if (rc.thereAreParticipants()) {
            //console.log('ÇEVİRİ GÖNDER', transcriptionData);
            rc.emitCmd(transcriptionData);
        }
    }

    handleTranscript(transcriptionData) {
        console.log('ÇEVİRİ METNİ', transcriptionData.text_data);
    ...
    togglePinUnpin() {
        if (rc.isChatPinned) {
            return userLog('info', 'Lütfen şu anda sabitlenmiş görünen sohbetin sabitlemesini kaldırın', 'top-end');
        }
    ...
    save() {
        if (this.transcripts.length != 0) {
            ...
            userLog('info', "Kaydedilecek çeviri yok", 'top-end');
        }
    }
    ...
    delete() {
        if (this.transcripts.length != 0) {
            Swal.fire({
                ...
                position: 'center',
                title: 'Tüm çeviriler temizlensin mi?',
            ...
            userLog('info', "Silinecek çeviri yok", 'top-end');
        }
    }
    ...
    start() {
        try {
            ...
            this.transcriptionRunning = false;
            userLog('error', `Çeviri başlatma hatası ${error.message}`, 'top-end', 6000);
            console.error('Çeviri başlatma hatası', error);
        }
    }
