...
module.exports = class Peer {
    ...
    // ####################################################
    // KATILIMCI BİLGİLERİNİ GÜNCELLEYİN
    // ####################################################

    updatePeerInfo(data) {
        log.debug('Katılımcı bilgilerini güncelle', data);
        switch (data.type) {
            case 'audio':
            case 'audioType':
    ...
    close() {
        this.transports.forEach((transport, transport_id) => {
            ...
            log.debug('Kapalı ve silinmiş katılımcı aktarımı', {
        ...
        log.debug('KATILIMCIYI KAPAT - AKTARIMLARI KONTROL EDİN | ÜRETİCİLER | TÜKETİCİLER', {
            peer_id: this.id,
    ...
    async createProducer(producerTransportId, producer_rtpParameters, producer_kind, producer_type) {
        ...
        if (['simulcast', 'svc'].includes(type)) {
            ...
            log.debug(`Üretici [${type}-${kind}] ----->`, {
                scalabilityMode,
                spatialLayer,
                temporalLayer,
            });
        } else {
            log.debug('Üretici ----->', { type: type, kind: kind });
        }

        producer.on('transportclose', () => {
            log.debug('Üretici "transportclose" etkinliği');
    ...
    closeProducer(producer_id) {
        ...
        try {
            ...
            log.warn('Üreticiyi kapat', error.message);
        }

        this.delProducer(producer_id);

        log.debug('Üretici kapatıldı ve silindi', {
            peer_name: this.peer_name,
            kind: kind,
            type: type,
            appData: appData,
    ...
    async createConsumer(consumer_transport_id, producer_id, rtpCapabilities) {
        ...
        const consumer = await consumerTransport.consume({
            ...
            rtpCapabilities,
            enableRtx: true, // OPUS için NACK'ı etkinleştir.
            paused: true,
        });
        ...
        if (['simulcast', 'svc'].includes(type)) {
            ...
            log.debug('Tüketici ----->', { type: type, kind: kind });
        }

        consumer.on('transportclose', () => {
            log.debug('Tüketici "transportclose" etkinliği');
    ...
    removeConsumer(consumer_id) {
        ...
        try {
            ...
            log.warn('Tüketiciyi kapat', error.message);
        }

        this.delConsumer(consumer_id);

        log.debug('Tüketici kapatıldı ve silindi', {
            peer_name: this.peer_name,
            kind: kind,
            type: type,
            consumer_id: id,
